...
export function walletHttp({ store, vault, service, status, publicOrigin = process.env.PUBLIC_ORIGIN }) {
  ...
  function password(user, value) {
    if (typeof value !== 'string' || value.length > 1024) throw new Error('Invalid password.');
    const key = passwordKey(value, Buffer.from(user.passwordSalt, 'base64url'));
    if (!passwordMatches(passwordHash(key), Buffer.from(user.passwordHash, 'base64url'))) { key.fill(0); throw Object.assign(new Error('Invalid account name or password.'), { status: 401 }); }
    return key;
  }
  /**
  ...
  const handler = async (req, res) => {
    ...
    try {
      ...
      if (post && ['/login','/register'].includes(p)) {
        ...
        if (!user) throw Object.assign(new Error('Invalid account name or password.'), { status: 401 });
      ...
      if (p.startsWith('/admin/')) {
        ...
        if (p === '/admin/lock' && post) { reauth(req,b); vault.db.prepare("INSERT OR REPLACE INTO settings VALUES ('locked',?)").run(b.locked === false ? '0' : '1'); return reply(res,200,{ok:true}); }
